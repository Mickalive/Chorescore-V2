...
if [[ "$criterion" == "V2-00" ]]; then
  ...
  native_hint="This is a targeted final-release repair cycle, not a product redesign. The Builder objective contains the GitHub Actions run id of the trusted release failure. Use GH_TOKEN and `gh run view <run-id> --repo \"$GITHUB_REPOSITORY\" --log-failed` to inspect the exact failing gate before editing. Repair only the concrete release/runtime/E2E blocker and any directly necessary tests or product code. Preserve V2-00..V2-06 behavior. You may edit product code, package/config, tests and scripts/; do not edit protected .github control-plane files. Prove the repair locally as far as the environment allows."
fi
